...
def run(run_id: str | None = None) -> None:
    ...
    for i, (model, action, trigger, conf, f1, _acc, prec, rec,
        ...
        improvement = (f1 - prev_score) if prev_score is not None else 0.0
        prev_score = f1
        ...
        _evidence = {"trigger": trigger, "confidence": round(conf, 2),
                     "train_val_gap": round(gap, 4), "primary_metric": round(f1, 4),
        ...
        sel = _selected(run_id, iter_num, next_act, next_params, trigger, conf,
                        round(f1 * 0.95, 3), 0.70, round(f1 * 0.88, 3),
                                evidence=_evidence)
        _HPARAMS_MAP = {('RandomForestClassifier', 'switch_model'): {'n_estimators': 100, 'max_depth': None, 'random_state': 42}, ('RandomForestClassifier', 'add_preprocessing'): {'n_estimators': 100, 'max_depth': None, 'random_state': 42}, ('GradientBoostingClassifier', 'switch_model'): {'n_estimators': 100, 'max_depth': 3, 'learning_rate': 0.1, 'random_state': 42}, ('ExtraTreesClassifier', 'switch_model'): {'n_estimators': 100, 'max_depth': None, 'random_state': 42}, ('LogisticRegression', 'switch_model'): {'C': 1.0, 'max_iter': 1000, 'solver': 'lbfgs', 'random_state': 42}}
        ...
        ui_signals(signals)
        ui_result(f1, "F1", gap, f1=f1, runtime=rt)
    ...
    ui_summary(run_id, out_dir, best_score, "ExtraTreesClassifier", "f1_score", comparison_rows)
