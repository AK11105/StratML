"""Demo: Wine Quality Red — Multiclass, persistent underfitting, model escalation."""
...
ROWS, COLS = 1599, 12
TRAIN, VAL, TEST = 959, 320, 320

ITERS = [
    # RF: tl=0.0 vs vl=0.297 → overfitting on hard multiclass (verified)
    ("RandomForestClassifier",    "switch_model",           "bootstrap",          0.50, 0.6531, 0.6333, 0.6248, 0.6531, 0.0,    0.2969, 0.2969, 0.25,
     {"overfitting": "strong", "overfitting_confidence": 0.78},
     "switch_model", {"model_name": "GradientBoostingClassifier"}),

    # GBM default: tl=0.070 vs vl=0.366 → still overfitting (verified)
    ("GradientBoostingClassifier","switch_model",           "overfitting",        0.71, 0.6094, 0.608,  0.6089, 0.6094, 0.0699, 0.3656, 0.2957, 1.17,
     {"overfitting": "weak", "overfitting_confidence": 0.61},
     "increase_model_capacity", {"scale": 1.5}),

    # GBM n_estimators=200: marginal gain, gap persists
    ("GradientBoostingClassifier","increase_model_capacity","overfitting",        0.65, 0.6156, 0.6143, 0.6151, 0.6156, 0.0621, 0.3594, 0.2973, 1.13,
     {"overfitting": "weak", "overfitting_confidence": 0.52, "well_fitted": "weak", "well_fitted_confidence": 0.38},
     "switch_model", {"model_name": "ExtraTreesClassifier"}),

    # ExtraTrees: tl=0.0 vs vl=0.306 → overfitting, stagnating vs GBM (verified)
    ("ExtraTreesClassifier",      "switch_model",           "overfitting",        0.61, 0.6281, 0.6107, 0.6015, 0.6281, 0.0,    0.3063, 0.3063, 0.13,
...
DATASET_META = {
    ...
    "missing_ratio": 0.0,
    "class_distribution": {"3": 10, "4": 53, "5": 681, "6": 638, "7": 199, "8": 18},
...
def run(run_id: str | None = None) -> None:
    ...
    for i, (model, action, trigger, conf, primary, f1, prec, rec,
        ...
        sel = _selected(run_id, iter_num, next_act, next_params, trigger, conf,
                        round(primary * 0.95, 3), 0.72, round(primary * 0.88, 3),
                                evidence=_evidence)
        _HPARAMS_MAP = {('RandomForestClassifier', 'switch_model'): {'n_estimators': 100, 'max_depth': None, 'random_state': 42}, ('GradientBoostingClassifier', 'switch_model'): {'n_estimators': 100, 'max_depth': 5, 'learning_rate': 0.1, 'random_state': 42}, ('GradientBoostingClassifier', 'increase_model_capacity'): {'n_estimators': 200, 'max_depth': 5, 'learning_rate': 0.1, 'random_state': 42}, ('ExtraTreesClassifier', 'switch_model'): {'n_estimators': 100, 'max_depth': None, 'random_state': 42}, ('SVC', 'switch_model'): {'C': 1.0, 'kernel': 'rbf', 'gamma': 'scale'}}
